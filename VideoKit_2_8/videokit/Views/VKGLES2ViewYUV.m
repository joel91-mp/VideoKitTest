...
NSString *const stringShaderFragmentYUVBOB = SHADER_STRING
(
 precision highp float;
 varying highp vec2 texCoordOut;
 uniform sampler2D s_texture_y;
 ...
 uniform int       u_interlaced_field;
  ...
  void main()
 {
     ...
     if(u_interlaced_field > 0) {
         //deinterlaced rendering
         vec2 offsetY;
         float temp1 = mod(texCoordOut.y, 2.0*u_stepY);
         
         offsetY  = texCoordOut;
         offsetY.y -= (temp1 - u_stepY/2.0 + float(u_interlaced_field)*u_stepY);
         float bstep = step(u_stepY, temp1);
         y = (texture2D(s_texture_y, offsetY).r - 0.06274509804) * 1.164;
     } else {
         //progressive rendering
         y = (texture2D(s_texture_y, texCoordOut).r - 0.06274509804) * 1.164;
     }
     
     u = texture2D(s_texture_u, texCoordOut).r - 0.5;
     v = texture2D(s_texture_v, texCoordOut).r - 0.5;
     
     r = y + 1.596 * v;
     g = y - 0.391 * u - 0.813 * v;
     b = y + 2.018 * u;
     
     vec4 rgba = vec4(vec3(r,g,b),1.0);
...
@interface VKGLES2ViewYUV () {
    ...
    float _stepY;
    int _interlacedField;
    BOOL _useDeinterlacer;
}
...
- (void)renderFrame:(VKVideoFrameYUV *)vidFrame {
    ...
    _useDeinterlacer |= vidFrame.interlaced;
    
    if (_useDeinterlacer) {
        glUniform1f(_stepY, 1.0/frameHeight);
        glUniform1i(_interlacedField, 1);
    } else {
        glUniform1i(_interlacedField, 0);
    }
...
- (int)compileShaderAndLinkProgram {
    ...
    _fragmentShader = [super compileShader:stringShaderFragmentYUVBOB
    ...
    _stepY = glGetUniformLocation(_program, "u_stepY");
    _interlacedField = glGetUniformLocation(_program, "u_interlaced_field");
