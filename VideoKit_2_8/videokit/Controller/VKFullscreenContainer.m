...
- (id)initWithPlayerController:(VKPlayerControllerBase *)player windowRect:(CGRect)rect {
    ...
    if (self) {
        ...
        _disableiPhoneXFullScreenToEmbedAnimationBugFix = NO;
    }
    return self;
}
...
- (void)dismissContainerWithAnimated:(BOOL)animated completionHandler:(void (^)(void))completionHandler {
    ...
    BOOL disableScrollViewCentering = NO;
    
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone && maxLength == 812.0 /* If dev is iPhoneX */) {
        if (_disableiPhoneXFullScreenToEmbedAnimationBugFix == NO) {
            disableScrollViewCentering = YES;
        }
    }
    
    if (disableScrollViewCentering == NO) {
