...
- (void)createUIFullScreenBar {
    ...
    [self insertBackgroundBlurViewTo:_toolBar];
    
    /* Toolbar on top: _viewBarContainer */
    _viewBarContainer = [[[UIView alloc] initWithFrame:CGRectZero] autorelease];
    _viewBarContainer.translatesAutoresizingMaskIntoConstraints = NO;
    _viewBarContainer.backgroundColor = [UIColor clearColor];
    [_toolBar addSubview:_viewBarContainer];
    
    float heightSubviewOnBar = 21.0;
    float wStrmTimeLabelsOnBar = 40.0;
    
    /* Toolbar on top: buttonDone */
    ...
    [_viewBarContainer addSubview:buttonDone];
    ...
    [_viewBarContainer addSubview:_labelBarTitle];
    ...
    [_viewBarContainer addSubview:_activityIndicator];
    ...
    [_viewBarContainer addSubview:_labelStreamCurrentTime];
    
    /* labelStreamTotalDuration */
    _labelStreamTotalDuration = [[[UILabel alloc] initWithFrame:CGRectMake(_viewBarContainer.frame.size.width - wStrmTimeLabelsOnBar - marginX, 6.0, wStrmTimeLabelsOnBar, heightSubviewOnBar)] autorelease];
    ...
    [_viewBarContainer addSubview:_labelStreamTotalDuration];
    ...
    _labelStreamCurrentTime.textColor = [UIColor darkGrayColor];
    [_viewBarContainer addSubview:_sliderCurrentDuration];
    ...
    [_viewBarContainer addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-55-[_labelBarTitle]-47-|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(_labelBarTitle)]];
    // align _labelBarTitle from the top
    [_viewBarContainer addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-6-[_labelBarTitle(==bar_subview_height)]" options:0 metrics:metricsBarSubviews views:NSDictionaryOfVariableBindings(_labelBarTitle)]];
    ...
    [_viewBarContainer addConstraint:[NSLayoutConstraint constraintWithItem:_activityIndicator attribute:NSLayoutAttributeCenterX relatedBy:NSLayoutRelationEqual toItem:_viewBarContainer attribute:NSLayoutAttributeCenterX multiplier:1.0 constant:-(textSizeTitle.width/2.0 + leftMarginActivityIndicator)]];
    // align _activityIndicator from the top
    [_viewBarContainer addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-7-[_activityIndicator]" options:0 metrics:nil views:NSDictionaryOfVariableBindings(_activityIndicator)]];
    
    // align _viewBarContainer from the left and right
    [_toolBar addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-0-[_viewBarContainer]-8-|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(_viewBarContainer)]];
    // align _viewBarContainer from the top
    NSLayoutConstraint *viewCenteredConstraintHeight = [NSLayoutConstraint constraintWithItem:_viewBarContainer attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:NULL attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:33.0];
    [_toolBar addConstraint:viewCenteredConstraintHeight];
    
    NSLayoutConstraint *viewCenteredConstraintTop =  [NSLayoutConstraint constraintWithItem:_viewBarContainer attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:_toolBar attribute:NSLayoutAttributeTop multiplier:1.0 constant:0.0];
    [_toolBar addConstraint:viewCenteredConstraintTop];
    
    // align buttonDone from the top
    [_viewBarContainer addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-0-[buttonDone(==30)]" options:0 metrics:nil views:NSDictionaryOfVariableBindings(buttonDone)]];
    
    // align subviews of viewBarContainer from the left & top
    [_viewBarContainer addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-8-[buttonDone(==44)]-8-[_labelStreamCurrentTime(==bar_time_label_width)]-4-[_sliderCurrentDuration]-4-[_labelStreamTotalDuration(==bar_time_label_width)]-8-|" options:NSLayoutFormatAlignAllCenterY metrics:metricsBarSubviews views:NSDictionaryOfVariableBindings(buttonDone, _labelStreamCurrentTime, _sliderCurrentDuration, _labelStreamTotalDuration)]];
}
...
- (void)createUIFullScreenPanel {
    ...
    [self insertBackgroundBlurViewTo:_viewControlPanel];
...
- (void)insertBackgroundBlurViewTo:(UIView *)view {
    if ([[[UIDevice currentDevice] systemVersion] compare:@"8.0" options:NSNumericSearch] != NSOrderedAscending) {
        ...
        if (!UIAccessibilityIsReduceTransparencyEnabled()) {
            ...
            [view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-0-[blurEffectView]-0-|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(blurEffectView)]];
            
            // align blurEffectView from the top and bottom
            [view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-0-[blurEffectView]-0-|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(blurEffectView)]];
        }
        else {
...
- (void)addUIFullScreen {
    if (![_viewControlPanel superview]) {
        ...
        [_viewControlPanel.centerXAnchor constraintEqualToAnchor:self.view.centerXAnchor constant:0.0].active = YES;
        [_viewControlPanel.heightAnchor constraintEqualToConstant:93.0].active = YES;
        [_viewControlPanel.widthAnchor constraintEqualToConstant:314.0].active = YES;
        
        if (@available(iOS 11.0, *)) {
            //running on iOS 11.0 or higher
            [_viewControlPanel.bottomAnchor constraintEqualToAnchor:self.view.safeAreaLayoutGuide.bottomAnchor constant:5.0].active = YES;
        } else {
            // Fallback on earlier versions
            [_viewControlPanel.bottomAnchor constraintEqualToAnchor:self.view.layoutMarginsGuide.bottomAnchor constant:5.0].active = YES;
        }
#else
    ...
    if (![_toolBar superview]) {
        ...
        [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-0-[_toolBar]" options:0 metrics:nil views:NSDictionaryOfVariableBindings(_toolBar)]];
        
        //_toolBar height constant
        NSLayoutConstraint *toolBarConstraintHeight = [NSLayoutConstraint constraintWithItem:_toolBar attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:NULL attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:44.0];
        toolBarConstraintHeight.identifier = @"toolbar-height-constraint";
...
- (void)updateBarHeight {
    ...
    NSLayoutConstraint *constraitToolBarHeight = nil;
    for (NSLayoutConstraint *constraint in constraints) {
        if ([constraint.identifier isEqualToString:@"toolbar-height-constraint"]) {
            constraitToolBarHeight = constraint;
            break;
        }
    }
    
    NSLayoutConstraint *constraitCenterViewTop = nil;
    for (NSLayoutConstraint *constaint in _toolBar.constraints) {
        if (constaint.firstItem == _viewBarContainer) {
            if (constaint.firstAttribute == NSLayoutAttributeTop) {
                constraitCenterViewTop = constaint;
                break;
            }
        }
    }
    ...
    float extraMargin = 0.0;
    float hasNotch = NO;
    
    if (@available(iOS 11.0, *)) {
        
        BOOL rotCurrPortrait = (screenSize.height > screenSize.width) ? YES : NO;
        float maxLength = MAX(screenSize.height, screenSize.width);
        ...
        if (hasNotch && rotCurrPortrait) {
            extraMargin = 32.0;
        }
    }
    
    float heightToolBar = 44.0 + extraMargin;
    float topCenteredView = 6.0 + extraMargin;
    
    if (_statusBarHidden || hasNotch) {
        constraitToolBarHeight.constant = heightToolBar;
        constraitCenterViewTop.constant = topCenteredView;
    } else {
        constraitToolBarHeight.constant = (statusBarHeight != 0) ? (statusBarHeight + heightToolBar) : (20.0 + heightToolBar);
        constraitCenterViewTop.constant = (statusBarHeight != 0) ? (statusBarHeight + topCenteredView) : (20.0 +topCenteredView);
    }
}

- (UIViewController *)topViewController:(BOOL)shouldBeVisible {
    UIViewController *currentVc = [[[[UIApplication sharedApplication] delegate] window] rootViewController];
    if (currentVc) {
        int try = shouldBeVisible ? 10 : 1;
        do {
...
- (void)useContainerViewControllerAnimated:(BOOL)animated {
    ...
    UIViewController *topVc = [self topViewController:NO];
    if (!topVc)
    return;
...
- (void)onRotation:(NSNotification *)theNotification {
    ...
    if (_mainScreenIsMobile) {
        [_scrollView updateForRotationChange];
    }
    [self updateBarHeight];
}
...
- (void)decoderStateChanged:(VKDecoderState)state errorCode:(VKError)errCode {
    ...
    if (state == kVKDecoderStateConnecting) {
        ...
        if (_controlStyle == kVKPlayerControlStyleFullScreen) {
            UIViewController *topVc = [self topViewController:YES];
            if (topVc) {
                ...
                [alert addAction:[UIAlertAction actionWithTitle:TR(@"OK") style:UIAlertActionStyleCancel handler:NULL]];
...
- (void)screenDidChange:(NSNotification *)notification forceDeattached:(BOOL)deattach {
    ...
    if (screenCount > 1 && !deattach) {
        ...
        self.extScreen.overscanCompensation = UIScreenOverscanCompensationNone;
        ...
        [_renderView updateOpenGLFrameSizes];
        [_renderView setStopUpdateGLSize:YES];
        // Add the GL view
        ...
        [_renderView setStopUpdateGLSize:NO];
