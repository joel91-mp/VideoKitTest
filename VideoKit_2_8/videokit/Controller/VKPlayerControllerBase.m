...
@synthesize isDecodeForManchester;
...
- (void)play {
    ...
    dispatch_async(_playStopQueue, ^(void) {
        ...
        if (_decodeManager) {
            ...
            _decodeManager.enableICYMetadataAutoFetch = _enableICYMetadata;
...
- (void)stop {
    ...
    if (isDecodeForManchester) {
        NSLog(@"22::Stop streaming");
        [_decodeManager sendRTSPCloseMessage];
    }
//#ifdef FIX_FOR_RTSP_TEARDOWN_MESSAGE
    //[_decodeManager sendRTSPCloseMessage];
   // NSLog(@"sendRTSPCloseMessage called");
//#endif
...
- (void)changeVideoStream {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [_decodeManager cycleVideoStream];
    });
}
...
#pragma mark - Shoutcast/ICY Metadata delegate method

- (void)onICYMetadataChanged:(VKICYMetadata *)icyMetadata {
    if(_delegate && [_delegate respondsToSelector:@selector(player:didICYMetadataChanged:)]) {
        [_delegate player:self didICYMetadataChanged:icyMetadata];
    }
}
