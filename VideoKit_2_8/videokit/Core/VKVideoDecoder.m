...
NSString *kVKVideoDataLost = @"VKVideoDataLost";
                    ...
                    if ((_frameLastPts != AV_NOPTS_VALUE) && (frame->pts != AV_NOPTS_VALUE)) {
                        ...
                        if (!isnan(clockdiff) && fabs(clockdiff) < AV_NOSYNC_THRESHOLD &&
                           !isnan(ptsdiff > 0) && ptsdiff < AV_NOSYNC_THRESHOLD &&
                            clockdiff + ptsdiff + [(VKDecodeManager *)_manager maxLatencyToDropVideoFrames] - _frameLastFilterDelay < 0 &&
                            [_pktQueue count]) {
    ...
    dispatch_async(_pictActionQueue, ^(void) {
        ...
        while (!_abortRequest) {
            ...
            if (!_refreshInProgress && (_forceRefresh ||
                ...
                BOOL doNotRefresh = _audioDecoder && [(VKDecodeManager *)_manager remoteFileStreaming] &&
                [_audioDecoder isWaitingForPackets];
...
- (void) refreshPicture {
    ...
    /// Following set of lines to calcualte the video packet size and  and notify the app if its zero for 5 seconds
    long  videoqsize;
    videoqsize = 0;
    videoqsize = _pktQueueSize;
    if(videoqsize == 0)
    {
        //As of now commenting this code for M4 version
        NSDictionary * dict = [NSDictionary dictionaryWithObjectsAndKeys:self.packetLostDate,@"packetRecievedDate",nil];
        [[NSNotificationCenter defaultCenter] postNotificationName:kVKVideoDataLost object:nil userInfo:dict];
        
    }
    else
    {
        self.packetLostDate = [NSDate date];
    }
            ...
            if (!last_time || (cur_time - last_time) >= 30000) {
                ...
                if (_stream && _stream->codec) {
                    av_log(NULL, AV_LOG_INFO,
                           "%7.2f %s:%7.3f fd= %d / %d aq=%5ldKB vq=%5ldKB f=%"PRId64"/%"PRId64"   \r",
...
- (UIImage *)snapshot {
    ...
    pthread_mutex_lock(&_mutexPictQueue);
    if (_pictQueue.count > 0) {
        
        int index = 0;
        
        if (_colorFormat == VKVideoStreamColorFormatYUVVT){
            VKVideoFrameYUVVT *vidFrameYUVVT = [_pictQueue objectAtIndex:index];
            
            CIImage *coreImage = [CIImage imageWithCVPixelBuffer:vidFrameYUVVT.pixelBuffer];
            CIContext *tempContext = [CIContext contextWithOptions:nil];
            CGImageRef imageRef = [tempContext createCGImage:coreImage fromRect:
                                   CGRectMake(0, 0, vidFrameYUVVT.width, vidFrameYUVVT.height)];
            
            image = [[[UIImage alloc] initWithCGImage:imageRef scale:1.0
                                                        orientation:UIImageOrientationRight] autorelease];
            CGImageRelease(imageRef);
        } else {
            
            AVPicture picture;
            avpicture_alloc(&picture, AV_PIX_FMT_RGBA, _codecContext->width, _codecContext->height);
            
            if (_colorFormat == VKVideoStreamColorFormatYUV) {
                
                VKVideoFrameYUV *vidFrameYUV = [_pictQueue objectAtIndex:index];
                ...
                pFrame->data[0] = vidFrameYUV.pLuma.data;
                pFrame->data[1] = vidFrameYUV.pChromaB.data;
                pFrame->data[2] = vidFrameYUV.pChromaR.data;
                
                pFrame->linesize[0] = vidFrameYUV.width;
                pFrame->linesize[1] = vidFrameYUV.width/2;
                pFrame->linesize[2] = vidFrameYUV.width/2;
                
                sws_scale(_imgConvertCtx, (const uint8_t **)pFrame->data, pFrame->linesize,
                          0, _codecContext->height, picture.data, picture.linesize);
                
                av_frame_free(&pFrame);

            } else if (_colorFormat == VKVideoStreamColorFormatRGB) {
                
                VKVideoFrameRGB *vidFrameRGB = [_pictQueue objectAtIndex:index];
                memcpy(picture.data[0], vidFrameRGB.pRGB.data, _codecContext->width * _codecContext->height);
            }
            
            NSUInteger width = _codecContext->width;
            NSUInteger height = _codecContext->height;
            NSInteger dataLength = width * height * 4;
            
            CGDataProviderRef dataProv = CGDataProviderCreateWithData(NULL, picture.data[0], dataLength, NULL);
            CGColorSpaceRef colorspace = CGColorSpaceCreateDeviceRGB();
            CGImageRef cgImage = CGImageCreate(width, height, 8, 32, width * 4, colorspace, kCGBitmapByteOrder32Big | kCGImageAlphaPremultipliedLast,
                                               dataProv, NULL, true, kCGRenderingIntentDefault);
            
            image = [[[UIImage alloc] initWithCGImage:cgImage] autorelease];
    ...
    if (image) {
        VKLog(kVKLogLevelDecoder, @"Snapshot action is successful");
    } else {
        VKLog(kVKLogLevelDecoder, @"Snapshot action is failed");
    }
