...
//#define VK_RECORDING_DEFAULT_FOLDER_PATH     [[NSHomeDirectory() stringByAppendingPathComponent: @"Documents"] stringByAppendingPathComponent:@"VideoStream"]]
#define VK_RECORDING_DEFAULT_FOLDER_PATH     [[NSHomeDirectory() stringByAppendingPathComponent:@"Documents"] stringByAppendingPathComponent: @"VideoStream"]

#define VK_RECORDING_DEFAULT_CONTAINER          @"mp4"
...
- (id)initWithInputFormat:(AVFormatContext *)fmtCtx activeAudioStreamId:(int)aStreamId
      activeVideoStreamId:(int)vStreamId fullPathWithFileName:(NSString *)path fileFormat:(NSString *)format {
    ...
    if (self) {
        ...
        fileFormat = format;
        if(0 == fileFormat.length)
        {
            fileFormat = VK_RECORDING_DEFAULT_CONTAINER;
        }
        [self initValues];
        [self initPath:path];
...
- (void)initPath:(NSString *)path {
    if (path && [path length]) {
        ...
        NSString * containerDefault = fileFormat;
...
- (void)stop {
    _abortRequest = YES;
    [self unlockQueues];
    //Malleswari - crash fix - _semaWritePktsThread should be released if all waits are signalled.. otherwise this results ins a crash..
    ...
    if (_semaWritePktsThread) {
        //NSLog(@"_semaWritePktsThread before wait:::%@",_semaWritePktsThread.debugDescription);
        if (dispatch_semaphore_wait(_semaWritePktsThread, 0) == 0) {
            //NSLog(@"_semaWritePktsThread after wait:::%@",_semaWritePktsThread.debugDescription);
            dispatch_release(_semaWritePktsThread);
            _semaWritePktsThread = NULL;
        }
    }
    ...
    for (int idStrm = 0; idStrm < _inputFmtCtx->nb_streams; idStrm++) {
        if (idStrm == _activeAudioStreamId || idStrm == _activeVideoStreamId) {
            ...
            if (in_stream->codec->codec_type == AVMEDIA_TYPE_AUDIO ||
                ...
                if (_outputFmtCtx && _outputFmtCtx->oformat) {
...
- (int)initCodecWithStream:(AVStream *)psout fromStream:(const AVStream *)sin {
    ...
    extra_size = (uint64_t)pccin->extradata_size + AV_INPUT_BUFFER_PADDING_SIZE;
    ...
    dispatch_async(_pktWriteQueue, ^(void) {
        ...
        for (;;) {
            ...
            if (!(_outputFmtCtx->oformat->flags & AVFMT_NOTIMESTAMPS) &&
                ...
                if (activeCodecCtx && activeCodecCtx->codec_type == AVMEDIA_TYPE_AUDIO) {
                ...
                } else if (activeCodecCtx && activeCodecCtx->codec_type == AVMEDIA_TYPE_VIDEO) {
...
- (void)dealloc {
    [_recordPath release];
    dispatch_release(_pktWriteQueue);
    [super dealloc];
}
