...
//                            } \
//                    } while (0)
//#define VKLog(A,frmt, ...) do {  \
//                            if (A & log_level) { \
//LOG_MAYBE(LOG_ASYNC_ENABLED, LOG_LEVEL_DEF, DDLogFlagVerbose, 2, nil, __PRETTY_FUNCTION__, frmt, ##__VA_ARGS__);  \
//                            } \
//                    } while (0)
...
typedef enum {
    ...
    kVKDecoderStateStoppedWithDataLost
} VKDecoderState;
 ...
 */
        - (id)initWithUsername:(NSString *)username secret:(NSString *)secret subScribeRequired:(BOOL)subscribeStatu;
...
/**
 *  Change to desired video stream with index if remote stream/file has more than 1 video stream
 *
 *  @param index Index of video stream
*/
...
///This structure holds the required vars and methods to active using custom I/O in ffmpeg.
...
/**
* This is a controlling property to limit reading data from resource.
*
* Default value is -1. -1 and 0 values are used for limiting
* data read from media stream and this is good for streams that can provide data rapidly(for example file streams.). On the
* other hand, setting this value to 1 and above removes limiting data controls and this is sometimes useful for realtime streaming.
*/
@property (nonatomic, assign) int infiniteBuffer;

///Specify YES to enable auto fetching shoutcast/icy metadata, default is NO.
@property (nonatomic, assign) BOOL enableICYMetadataAutoFetch;
        
///This is a time interval for polling metadata for http shoutcast & icy protocols, default value is 3.0 seconds.
@property (nonatomic, assign) float icyMetadataPollingInterval;
...
- (void)notifyPacketLost:(NSDate *)lastPacketRecievedDate;
#endif
        
/**
 *  Optional delegate method, notifies the observer about the metadata change event 
 *  for http shoutcast and icy protocols
 */
- (void)onICYMetadataChanged:(VKICYMetadata *)icyMetadata;
