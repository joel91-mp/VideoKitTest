...
NSString *VKDECODER_OPT_KEY_HW_ACCEL_DISABLED_AUDIO             = @"disable_hardware_acceleration_for_audio";
NSString *VKDECODER_OPT_KEY_HW_ACCEL_DISABLED_VIDEO             = @"disable_hardware_acceleration_for_video";
...
extern char *av_asprintf(const char *fmt, ...) av_printf_format(1, 2);
...
//AVIO inner buffer size, it's like avformatctx inner buffer size
#define AVIO_INNER_BUFFER_SIZE                      4096

//This is the max latency default value that VKVideoDecoder starts dropping video frames
#define VK_FF_DROP_VIDEO_FRAME_MAX_LATENCY          0.3

//This is the time interval default value in seconds for polling metadata of http shoutcast/icy protocols
#define VK_ICY_METADATA_POLLING_INTERVAL                3.0
...
@interface VKDecodeManager () {
    ...
    //Custom IO
    VKDecoderCustomIO *_customIO;
    AVIOContext *_avIOCtx;
    uint8_t *_avIOCtxBuffer;
    size_t _avIOCtxBufferSize;
    
    float _maxLatencyToDropVideoFrames;
    
    //ICY metadata
    BOOL _enableICYMetadataAutoFetch;
    NSTimer *_timerICYMetadataPolling;
    float _icyMetadataPollingInterval;
    NSString *_lastICYMetadataTitleRaw;
    NSString *_lastICYMetadataHeadersRaw;
}
...
@synthesize enableTogglePauseWithoutSendingNetworkMsg = _enableTogglePauseWithoutSendingNetworkMsg;
@synthesize enableCustomIO = _enableCustomIO;
@synthesize customIO = _customIO;
@synthesize maxLatencyToDropVideoFrames = _maxLatencyToDropVideoFrames;
@synthesize icyMetadataPollingInterval = _icyMetadataPollingInterval;
@synthesize enableICYMetadataAutoFetch = _enableICYMetadataAutoFetch;
...
- (id)initWithUsername:(NSString *)username secret:(NSString *)secret subScribeRequired:(BOOL)subscribeStatus {
    ...
    if(self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(notifyDataLost:) name:kVKVideoDataLost object:nil];
        }
...
- (void)initValues {
    ...
    _enableTogglePauseWithoutSendingNetworkMsg = NO;
    
    _disableInnerBuffer = NO;
    _enableCustomIO = NO;
    ...
    _avIOCtx = NULL;
    _avIOCtxBuffer = NULL;
    _avIOCtxBufferSize = AVIO_INNER_BUFFER_SIZE;
    
    _customIO = malloc(sizeof(VKDecoderCustomIO));
    memset(_customIO, 0, sizeof(VKDecoderCustomIO));
    
    _maxLatencyToDropVideoFrames = VK_FF_DROP_VIDEO_FRAME_MAX_LATENCY;
    
    _enableICYMetadataAutoFetch = NO;
    _icyMetadataPollingInterval = VK_ICY_METADATA_POLLING_INTERVAL;
}
...
- (VKError)connectWithStreamURLString:(NSString*)urlString options:(NSDictionary *)options {
    ...
    if (passThrough) {
        ...
        _streamURL = [self updateProtocolSchemeIfNeeded:url cache:useCache];
    ...
    if (_showStatus && [urlStringFinal length] == 0) { //Length added by naman
...
- (AVDictionary*)updateProtocolSettingsWithURL:(NSURL*) url {
    ...
    if ([[_streamURL scheme] isEqualToString:@"rtsp"]) {
        ...
        if ((of = av_opt_find(&fc, key, NULL, 0,
            ...
            av_dict_set(&option_dict, key, val, (FLAGS(of) | AV_DICT_DONT_STRDUP_KEY | AV_DICT_DONT_STRDUP_VAL));
        }
    }
    return option_dict;
}
...
- (VKError)openStreamURLString:(NSString *)urlString option:(AVDictionary *)option {
    ...
    if (_disableInnerBuffer) {
        _avFmtCtx->flags |= AVFMT_FLAG_NOBUFFER;
        _infiniteBuffer = 0;
    }
    
    if (_enableCustomIO) {
        _avFmtCtx->flags |= AVFMT_FLAG_CUSTOM_IO;
        
        _avIOCtxBuffer = av_malloc(_avIOCtxBufferSize);
        if (!_avIOCtxBuffer) {
            VKLog(kVKLogLevelDecoder, @"Buffer can not be allocated for CustomIO: %d", AVERROR(ENOMEM));
            return kVKErrorOpenStream;
        }
        
        _avIOCtx = avio_alloc_context(_avIOCtxBuffer, (int)_avIOCtxBufferSize,
                                      0, _customIO->opaque, _customIO->custom_io_read_data_cb, NULL, _customIO->custom_io_seek_cb);
        
        if (!_avIOCtx) {
            VKLog(kVKLogLevelDecoder, @"CustomIO: can not create aviocontext %d", AVERROR(ENOMEM));
            return kVKErrorOpenStream;
        }
        
        if ((*(_customIO->custom_io_open_cb))(_customIO->opaque) >= 0) {
            VKLog(kVKLogLevelDecoder, @"CustomIO: Success opening custom IO");
        } else {
            VKLog(kVKLogLevelDecoder, @"CustomIO: Stream can not be opened %d", AVERROR(ENOMEM));
            return kVKErrorOpenStream;
        }
    }
...
- (VKError)openAudioStreamWithId:(NSInteger)sId {
    ...
    if(audCdc->name &&
       ((strcasestr(audCdc->name, "PCM_ALAW") == NULL) &&
        (strcasestr(audCdc->name, "PCM_MULAW") == NULL) &&
        (strcasestr(audCdc->name, "MP2") == NULL))) {
    ...
    if(stream_lowres)
        audCdcCtx->flags |= AV_CODEC_FLAG_GRAY;
    
    if (_fast)
        audCdcCtx->flags2 |= AV_CODEC_FLAG2_FAST;
    
    if(audCdc->capabilities & AV_CODEC_CAP_DR1)
        audCdcCtx->flags |= AV_CODEC_FLAG_GRAY;
...
- (VKError)openVideoStreamWithId:(NSInteger)sId audioIsOK:(BOOL)audioIsOK {
    ...
    if(vidCdcCtx->lowres)
        vidCdcCtx->flags |= AV_CODEC_FLAG_GRAY;
    
    if (_fast)
        vidCdcCtx->flags2 |= AV_CODEC_FLAG2_FAST;

    if(vidCdc->capabilities & AV_CODEC_CAP_DR1)
        vidCdcCtx->flags |= AV_CODEC_FLAG_GRAY;
    
    if (_disableWaitingForFirstIntraFrame) {
        vidCdcCtx->flags2 |= AV_CODEC_FLAG2_SHOW_ALL;
    }

    opts = filter_codec_opts(codec_opts, vidCdcCtx->codec_id, _avFmtCtx, _avFmtCtx->streams[sId], vidCdc);
    
    if (!av_dict_get(opts, "threads", NULL, 0)) {
        if (vidCdcCtx->pix_fmt != AV_PIX_FMT_VIDEOTOOLBOX) {
            av_dict_set(&opts, "threads", "auto", 0);
        }
    }
    ...
    _vDecoder.packetLostDate = [NSDate date]; //[NSDate dateWithTimeIntervalSinceNow: -(60.0f*60.0f*24.0f)];
...
- (void)startRecordingInFormat:(NSString *)format {
    NSLog(@"startRecordingInFormat entered");
    ...
    _recorder = [[VKRecorder alloc] initWithInputFormat:_avFmtCtx activeAudioStreamId:audioStreamId
                                    activeVideoStreamId:videoStreamId fullPathWithFileName:NULL fileFormat:format];
    if (!_recorder) {
        NSLog(@"startRecordingInFormat: recorder is not there..hence returning");
        return;
    }
...
- (void)startRecording {
    ...
    _recorder = [[VKRecorder alloc] initWithInputFormat:_avFmtCtx activeAudioStreamId:audioStreamId
                                    activeVideoStreamId:videoStreamId fullPathWithFileName:NULL fileFormat:@"mp4"];
    if (!_recorder)
        return;
    _recorder.delegate = self;
    [_recorder start];
...
- (void)didStopRecordingWithRecorder:(VKRecorder *)recorder error:(VKErrorRecorder)error {
    ...
    __block VKRecorder *recorderToBeReleased = recorder;
    
    dispatch_async(dispatch_get_main_queue(), ^(void) {
        if(_delegate && [_delegate respondsToSelector:@selector(didStopRecordingWithPath:error:)]) {
            [_delegate didStopRecordingWithPath:[[[recorderToBeReleased recordPath] copy] autorelease] error:error];
        }
        
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [recorderToBeReleased setDelegate:nil];
            if (recorderToBeReleased == _recorder) {
                [_recorder release];
                _recorder = NULL;
            }
            else {
                NSLog(@"recorderToBeReleased is not _recorder");
                [recorderToBeReleased release];
                recorderToBeReleased = NULL;
            }
...
- (void) readPackets {
    ...
    dispatch_async(_readPktQueue, ^(void) {
        ...
        for (;;) {
            ...
            if (_enableTogglePauseWithoutSendingNetworkMsg == NO) {
            ...
            if (eof) {
                ...
                if (totalSize == 0) {
                    if (_loopPlayback != 1 && (!_loopPlayback || --_loopPlayback)) {
                        ...
                        int modVKRecurringNotCount = 1/DEFAULT_VK_RECURRING_NOTIFICATION_FREQUENCY;
                        
                        if ((_vkRecNotIter%modVKRecurringNotCount == 0)) {
                            _vkRecNotIter = 0;
                            if (_decoderState == kVKDecoderStatePlaying) {
                                [self setDecoderState:kVKDecoderStatePlaying errorCode:kVKErrorNoneReachedEndOfStream];
                            }
                        }
                        _vkRecNotIter++;
                    }
                ...
                if (err < 0 || playingTimeExpired) {
                    if (err == AVERROR_EOF || /*url_feof(_avFmtCtx->pb) // old API*/avio_feof(_avFmtCtx->pb)) {
                        ...
                        if ((_pktReadErrIter%modPktReadErrCount == 0)) {
                            _pktReadErrIter = 0;
                            VKLog(kVKLogLevelDecoderExtra, @"End of file warning, there is no packets left to read");
                        }
                        _pktReadErrIter++;
                    ...
                    if (!_abortIsRequested) {
                        ...
                        if (_initialAVSync && _audioIsOK && _videoIsOK) {
                            ...
                            for (int iterAudioPkt = (int)([[_aDecoder pktQueue] count] -1); iterAudioPkt >= 0; iterAudioPkt--) {
                                ...
                                if (!closedAudPktFound) {
                                    VKLog(kVKLogLevelDecoderExtra, @"AVDIFF: %f - AudioPkt is removed (pkt.pts: %f) >>>>>", diff, (((int64_t)pkt.pts) * av_q2d(_avFmtCtx->streams[[_aDecoder streamId]]->time_base)));
                                    [_aDecoder removePktAtIndex:iterAudioPkt];
                                } else {
...
- (void)setLogLevel:(VKLogLevel)logLevel {
    ...
    } else if (log_level == kVKLogLevelAllWithFFMPEG) {
        opt_loglevel(NULL, "loglevel", "trace");
    }
}
...
//#if CONFIG_RTSP_DEMUXER || CONFIG_MMSH_PROTOCOL
   if(_avFmtCtx && _avFmtCtx->iformat && !strcmp(_avFmtCtx->iformat->name, "rtsp")) {
       RTSPState *r = (RTSPState *)_avFmtCtx->priv_data;
        NSInteger length = strlen(r->session_id);
       if (length > 0){
           ff_rtsp_send_cmd_async(_avFmtCtx, "PAUSE", r->control_uri, NULL);
           NSLog(@"sendRTSPCloseMessage condition called");
           ff_rtsp_send_cmd_async(_avFmtCtx, "TEARDOWN", r->control_uri, NULL);
       }
       else{
           NSLog(@"Session id empty, RTSP TEARDOWN will not be sent");
       }
    }
//#endif
}
...
- (void)abort {
    ...
    [self stopTrialTimer];
#endif
    [self stopICYMetadataPollingTimer];
...
- (void)doSeek:(double)value {
    ...
    double pos = 0.0;
    int valueUsedBySeekByBytes = value;
    ...
    if (_seekByBytes) {
        ...
        if (_avFmtCtx->bit_rate)
            valueUsedBySeekByBytes *= _avFmtCtx->bit_rate / 8.0;
        else
            valueUsedBySeekByBytes *= 180000.0;
        [self streamSeek:valueUsedBySeekByBytes rel:valueUsedBySeekByBytes byBytes:1];
    } else {
        pos = value;
...
- (void)cycleVideoStream {
    ...
    [self stopRecording];
#endif
    [self cycleStreamWithType:AVMEDIA_TYPE_VIDEO index:-1];
    [self togglePause];
}

- (void)cycleVideoStreamWithStreamIndex:(int) index {
    [self togglePause];
#ifdef VK_RECORDING_CAPABILITY
    [self stopRecording];
#endif
    [self cycleStreamWithType:AVMEDIA_TYPE_VIDEO index:index];
    [self togglePause];
}
...
- (void)setDecoderState:(VKDecoderState)state errorCode:(VKError)errCode {
    ...
    if ((_decoderState != state) || (errCode == kVKErrorNoneReachedEndOfStream)) {
        _decoderState = state;
        dispatch_async(dispatch_get_main_queue(), ^(void) {
...
#endif
                if (_enableICYMetadataAutoFetch) {
                    [self onTimerICYMetadataPollingFired:nil];
                    [self startICYMetadataPollingTimer];
                }
            }
        });
    }
...
#pragma mark - Video Data Lost callback

- (void) notifyDataLost:(NSNotification *)notification
{

    NSDictionary * dict = (NSDictionary *)notification.userInfo;
    NSDate * packetRecievedDate = [dict valueForKey:@"packetRecievedDate"];
    if(_delegate && [_delegate respondsToSelector:@selector(notifyPacketLost:)]) {
        [_delegate notifyPacketLost:packetRecievedDate];
        }
//    if(_delegate && [_delegate respondsToSelector:@selector(streamDataLost)]) {
//        [_delegate streamDataLost];
    //}
}
...
- (BOOL)isRealTime {
    if (_avFmtCtx != NULL) {
        ...
        if (!strncmp(_avFmtCtx->filename, "rtmp", 4)) {
            char *ext = strrchr(_avFmtCtx->filename, '.');
            if (!ext) {
                /* no extension */
                return YES;
            } else {
                if (!strcasecmp((ext+1), "flv")) {
                    return NO;
                } else {
                    return YES;
                }
...
#pragma mark - Metadata For Http Shoutcast/ICY Protocols

- (void)startICYMetadataPollingTimer {

    [self stopICYMetadataPollingTimer];
    _timerICYMetadataPolling = [[NSTimer scheduledTimerWithTimeInterval:_icyMetadataPollingInterval target:self selector:@selector(onTimerICYMetadataPollingFired:) userInfo:nil repeats:YES] retain];
    [[NSRunLoop mainRunLoop] addTimer:_timerICYMetadataPolling forMode:NSRunLoopCommonModes];
}

- (void)stopICYMetadataPollingTimer {
    if (_timerICYMetadataPolling && [_timerICYMetadataPolling isValid]) {
        [_timerICYMetadataPolling invalidate];
    }
    [_timerICYMetadataPolling release];
    _timerICYMetadataPolling = nil;
}

- (void)onTimerICYMetadataPollingFired:(NSTimer *)timer {
    
    NSString *strTitle = @"";
    NSString *strHeaders = @"";
    
    char* metadata = NULL;
    av_opt_get(_avFmtCtx, "icy_metadata_packet", AV_OPT_SEARCH_CHILDREN, (uint8_t**) &metadata);
    if (metadata!= NULL && strlen(metadata) != 0) {
        strTitle = [NSString stringWithCString:metadata encoding:NSUTF8StringEncoding];
        VKLog(kVKLogLevelDecoderExtra, @"%@", strTitle);
    }
    av_free(metadata);
    
    metadata = NULL;
    av_opt_get(_avFmtCtx, "icy_metadata_headers", AV_OPT_SEARCH_CHILDREN, (uint8_t**) &metadata);
    if (metadata!= NULL && strlen(metadata) != 0) {
        strHeaders = [NSString stringWithCString:metadata encoding:NSUTF8StringEncoding];
        VKLog(kVKLogLevelDecoderExtra, @"%@", strHeaders);
    }
    av_free(metadata);
    
    BOOL anyChange = NO;
    
    if (_lastICYMetadataTitleRaw == NULL ||
        [_lastICYMetadataTitleRaw isEqualToString:strTitle] == NO) {
        [_lastICYMetadataTitleRaw release];
        _lastICYMetadataTitleRaw = [strTitle retain];
        anyChange = YES;
    }
    
    if (_lastICYMetadataHeadersRaw == NULL ||
               ![_lastICYMetadataHeadersRaw isEqualToString:strHeaders]) {
        [_lastICYMetadataHeadersRaw release];
        _lastICYMetadataHeadersRaw = [strHeaders retain];
        anyChange = YES;
    }
    
    if (anyChange) {
        VKICYMetadata *icyMetadata = [[[VKICYMetadata alloc] initWithTitle:strTitle headersRaw:strHeaders] autorelease];
        if(_delegate && [_delegate respondsToSelector:@selector(onICYMetadataChanged:)]) {
            [_delegate onICYMetadataChanged:icyMetadata];
        }
    }
}
...
- (void)shutDown {
    ...
    if (_enableCustomIO) {
        (*(_customIO->custom_io_close_cb))(_customIO->opaque);
    }
}
