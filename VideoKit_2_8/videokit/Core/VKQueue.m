...
- (void)removeLastPkt {
    if ([_pktQueue count]) {
        int size = [(VKPacket *)[_pktQueue lastObject] size];
        _pktQueueSize -= size;
...
- (void)removePktAtIndex:(unsigned int)index {
    if ([_pktQueue count] > index) {
        int size = [(VKPacket *)[_pktQueue objectAtIndex:index] size];
        _pktQueueSize -= size;
        [_pktQueue removeObjectAtIndex:index];
    }
}
