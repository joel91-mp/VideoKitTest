...
- (id)initWithURLString:(NSString *)urlString decoderOptions:(NSDictionary *)options {
    ...
    if (self) {
        ...
        _urlString = @"rtsp://admin:12345@192.168.1.246/MediaInput/h264/stream_1";//[urlString retain];
...
- (void)setEnableICYMetadata:(BOOL)enableICYMetadata {
    _enableICYMetadata = enableICYMetadata;
    _playerController.enableICYMetadata = _enableICYMetadata;
}
...
- (void)player:(VKPlayerControllerBase *)player didICYMetadataChanged:(VKICYMetadata *)icyMetadata {
    if(_delegate && [_delegate respondsToSelector:@selector(onPlayerViewControllerDidICYMetadataChanged:)]) {
        [_delegate onPlayerViewControllerDidICYMetadataChanged:icyMetadata];
    }
}
